...
class Hotword extends \app\admin\controller\Common
{
    ...
    public function add()
    {
        if (request()->isGet()) {
            ...
            if (\think\Db::name('content_hotword')->where('tag', 'eq', input('tag'))->find()) {
                $this->error('当前标签已经存在！');
            }
    ...
    public function edit()
    {
        if (request()->isGet()) {
            ...
            if (\think\Db::name('content_hotword')->where($where)->find()) {
                $oldtag = \think\Db::name('content_hotword')->find(input('id'));
    ...
    public function merge()
    {
        if (request()->isGet()) {
            ...
            if ($tag1 && $tag2 && $tag1[0] != $tag2) {
                ...
                $where['tag'] = ['in', $tag1];
                if ($tag1_ids = \think\Db::name('content_hotword')->where($where)->column('id')) {

                    // 判断tag2是否存在，不存在就创建
                    if (!$tag2_id = \think\Db::name('content_hotword')->where(['tag' => $tag2])->value('id')) {
                        // 创建tag2 并获取其id
                        $data = [
                            'tag' => $tag2,
                            'count' => 0,
                    ...
                    // 删除旧数据
                    \think\Db::name('content_hotword')->where(['id' => ['in', $tag1_ids]])->delete();
                    $this->success('合并成功！');
                }
            }
            $this->error('操作失败！');
