...
	</section>    <script type="text/javascript">	var url = 'http://120.26.192.83/index.php/index/comment/list_data?pid='+$('#category_id').val()+'&page=';	function addIndexHtml(data){		var html = '';		for(var i=0;i < data.length;i++){			var tag = '';	    	if(data[i].tag){	    		tag = data[i].tag;	    	}			html+= '<li><a href="'+data[i].url+'">'            	+'<img class="newsList_img" src="'+data[i].thumb+'">'            	+'<div class="newsList_d">'            	+'	<P class="newsList_d_p1">'+tag+'</P>'            	+'    <P class="newsList_d_p2"><span>'+data[i].title+'</span></P>'            	+'    <P class="newsList_d_p3">By '+data[i].author+'</P>'            	+'    <P class="newsList_d_p4">'+data[i].description+'</P>'            	+' </div>'            	+' </a></li>';		}		$('#data_list').append(html);	}</script>
